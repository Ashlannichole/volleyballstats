  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={openAdd}
          className="tap-btn bg-vr-600 hover:bg-vr-500 text-white font-bold px-5 py-3 rounded-xl text-lg"
        >
          + Add Player
        </button>
      </div>
      ...
      <div className="space-y-3">
        {sorted.map(p => (
          <div key={p.id} className="bg-navy-700 border border-white/10 rounded-2xl p-4 flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-vr-800 border-2 border-vr-500/40 flex items-center justify-center text-xl font-bold text-pb-400 shrink-0">
              #{p.number}
            </div>
            ...
            <div className="flex gap-2 shrink-0">
              <button onClick={() => openEdit(p)} className="tap-btn text-pb-400 hover:text-pb-300 px-3 py-2 text-sm font-medium">
                Edit
              </button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 p-4">
          <div className="bg-navy-800 border border-vr-700/50 rounded-2xl p-6 w-full max-w-md">
            <h3 className="text-xl font-bold text-white mb-5">{editing ? 'Edit Player' : 'Add Player'}</h3>

            <label className="block text-gray-300 text-sm mb-1">Name</label>
            <input
              className="w-full bg-navy-600 border border-white/20 rounded-xl px-4 py-3 text-white text-lg mb-4 outline-none focus:border-pb-500"
              value={form.name}
              ...
              {POSITIONS.map(pos => (
                <button
                  ...
                  className={`tap-btn py-3 rounded-xl font-bold text-sm border-2 transition-colors ${
                    form.position === pos
                      ? 'border-vr-500 bg-vr-700 text-white'
            ...
            <div className="flex gap-3">
              ...
              <button onClick={save} className="tap-btn flex-1 py-3 rounded-xl bg-vr-600 hover:bg-vr-500 text-white font-bold text-lg">
                Save
              </button>
            </div>
          </div>
