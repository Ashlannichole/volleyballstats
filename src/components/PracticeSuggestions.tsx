...
${JSON.stringify(payload, null, 2)}`
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <button onClick={onBack} className="tap-btn text-blue-400 text-sm mb-4 flex items-center gap-1">
        ← Match History
      </button>

      <h2 className="text-2xl font-bold text-white mb-1">AI Practice Suggestions</h2>
      <p className="text-gray-400 text-sm mb-4">vs {match.opponent} · {match.date}</p>

      {/* Team snapshot */}
      <div className="bg-navy-700 border border-white/10 rounded-2xl p-4 mb-6">
        <p className="text-gray-400 text-xs font-bold uppercase mb-3">Match Summary</p>
      ...
      {!suggestions && !loading && (
        <button
          ...
          className="tap-btn w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-5 rounded-2xl text-lg"
        >
      ...
      {error && (
        <div className="bg-red-900/30 border border-red-500/30 rounded-2xl p-4 mb-4">
          ...
          <button onClick={fetchSuggestions} className="tap-btn text-blue-400 text-sm mt-2">
            Try again
          </button>
        </div>
      )}
      ...
      {suggestions && (
        <div className="space-y-3">
          {suggestions.map((s, i) => (
            <div key={i} className="bg-navy-700 border border-white/10 rounded-2xl p-4">
              <div className="flex items-start gap-3 mb-2">
                <span className="bg-blue-600 text-white font-bold text-sm w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5">
                  {i + 1}
                </span>
                <div>
                  <p className="text-white font-bold text-lg">{s.title}</p>
                  <p className="text-blue-300 text-xs font-medium mb-2">Focus: {s.focus}</p>
